...
MovieInfo <- function (movie.name){
    ...
    params <- list('api-key' = NEW.YORK.TIMES.KEY, query = movie.name)

    # Send the HTTP Request to download the data
    # Extract the content and convert it from JSON
    data <- fromJSON(content(GET(paste(BASE, resource, sep = ''), query = params), 'text'), flatten = TRUE)
